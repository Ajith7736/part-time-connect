...
function Adminusers() {
  ...
  const addblocklist = async (Email) => {
    const res = await fetch(`${BASE_URL}/api/admin/addblocklist`,{
      method : "POST",
      headers : {
  ...
  const deleteuserdata = async (Email) => {
    const res = await fetch(`${BASE_URL}/api/admin/deleteuser`,{
      method : "POST",
      headers : {
        "Content-Type" : "application/json"
      },
  ...
  const handledailog = (Email) => {
    setdailogbox(true)
    setuseremail(Email)
  }

  const handledelete = () => {
    useremail && addblocklist(useremail);
          ...
          <div className='fixed z-1 lg:left-[40%]'>
            <div className='bg-white h-[30vh] max-[450px]:h-[25vh] max-[390px]:h-[35vh] w-[80vw] lg:w-[30vw] shadow-md rounded-xl p-10 flex flex-col items-center justify-center gap-8'>
              ...
              <div className='flex gap-8'>
                ...
                <button className='bg-red-500 hover:bg-red-600 p-2 rounded-xl cursor-pointer font-medium text-white max-[450px]:text-xs' onClick={handledelete}>Yes, I'm sure</button>
              </div>
            </div>
          </div>
        </>}
            ...
            <tbody>
              {userdata.map((item, index) => {
                return <tr key={index} className="bg-white border-b  border-gray-200">
                  ...
                  <td className='px-8'>
                    <RiDeleteBin5Line size={20} className='cursor-pointer hover:text-red-500' onClick={()=> {handledailog(item.Email)}} />
                  </td>
                </tr>
              })}
